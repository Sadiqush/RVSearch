...
def video_init(vid_info):
    ...
    if not config.QUIET: print(f"Loading video: {vid_meta['path']} -- {vid_meta['name']}")
    start = time()
    frames = Video(vid_meta['path'], fps=1).get_frames()
    frames = get_frames(vid, vid_meta['path'])
    if not config.QUIET: print(time() - start)
...
def compare_videos(source_frames, source_fps, target_frames, target_fps):
    ...
    if not config.QUIET: print('Getting ready to start comparison process')
    ...
    if not config.QUIET: print('**Comparing started**')
    ...
    if config.VERBOSE: print("--- %s seconds ---" % (time() - start))
    return timestamps
