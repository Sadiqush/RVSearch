...
def video_init(vid_info):
    ...
    if not vconf.QUIET: print(f"Loading video: {vid_meta['path']} -- {vid_meta['name']}")
    start = time()
    ...
    for s_frame in source_frames:
        ...
        for t_frame in target_frames:
            ...
            if check_score(score, threshold=0.75):
                ...
                if vconf.VERBOSE:
                    print(timestamps[-1])
                    # print(f"{timestamps[0][0]}:{timestamps[0][1]} - {timestamps[1][0]}:{timestamps[1][1]}
                    # score: {timestamps[0][2]}")
